...
from pipeline.preprocess import filter_signal, segment_signal, _refine_ao_peak
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Input(id="highcut", type="number", min=10, max=250, step=1, value=200, debounce=True),
    ...
    html.Div([
        ...
        # First-pass detection parameters
        html.H3("Pass 1 — Beat Detection", style={"marginTop": "10px", "color": "#444"}),
        html.Label("Distance (samples): "),
        dcc.Input(id="distance", type="number", min=10, max=500, step=1, value=int(Fs/3), debounce=True),
        html.Label("  Prominence: "),
        dcc.Input(id="prominence", type="number", min=0, max=1, step=0.001, value=0.02, debounce=True),
        html.Label("  Tolerance (IQR multiplier): "),
        dcc.Input(id="tolerance", type="number", min=0, max=10, step=0.1, value=1, debounce=True),

        # Second-pass AO/MV refinement parameters
        html.H3("Pass 2 — AO/MV Refinement", style={"marginTop": "15px", "color": "#444"}),
        html.Label("Enable refinement: "),
        dcc.Checklist(
            id="ao-mv-refinement",
        ...
        html.Label("Local search radius (samples): "),
        dcc.Input(id="local-search-radius", type="number", min=5, max=100, step=1, value=25, debounce=True),
        html.Label("  Local distance (samples): "),
        dcc.Input(id="local-distance", type="number", min=1, max=20, step=1, value=5, debounce=True),
        html.Label("  Similarity threshold: "),
        dcc.Input(id="similarity-threshold", type="number", min=0.1, max=1.0, step=0.05, value=0.6, debounce=True),
    ...
    html.Div([
        ...
        dcc.Input(id="segment-width", type="number", min=50, max=300, step=1, value=300, debounce=True),
        html.Label("Averaging window: "),
        dcc.Input(id="averaging-window", type="number", min=0, max=20, step=1, value=5, debounce=True),
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Input(
                id="reference-paths",
                type="text",
                value="data/references/ref21_reference.csv",
                style={"width": "80%"},
                debounce=True
            ),
        ], style={"margin": "10px"}),
    ...
    dcc.Store(id="refined-peaks-data"),
...
@app.callback(
    [Output("downpeaks", "figure"),
     ...
     Output("refined-peaks-data", "data")],
    [Input("filtered-data", "data"),
     Input("distance", "value"),
     Input("prominence", "value"),
     Input("tolerance", "value"),
     Input("ao-mv-refinement", "value"),
     Input("local-search-radius", "value"),
     Input("local-distance", "value"),
     Input("similarity-threshold", "value"),
     Input("downpeaks", "relayoutData")]
)
def update_downpeaks(
    z_filt_data, distance, prominence, tolerance,
    ao_mv_refinement, local_search_radius, local_distance, similarity_threshold,
    relayout_data
):
    if z_filt_data is None:
        return px.line(title="No filtered data available"), [], [], []
    ...
    # --- Pass 1: beat-level detection ---
    downpeaks, _ = find_peaks(-z_filt, distance=int(distance), prominence=float(prominence))

    if len(downpeaks) == 0:
        return plot_time_series(t, z_filt, title="No down-peaks detected"), [], [], []
    ...
    filtered_peaks = [p for p in downpeaks if lower_cut <= z_filt[p] <= upper_cut]
    outlier_peaks  = [p for p in downpeaks if p not in filtered_peaks]

    # --- Pass 2: AO/MV refinement ---
    do_refine = "on" in (ao_mv_refinement or [])
    if do_refine and filtered_peaks:
        refined_peaks = [
            _refine_ao_peak(
                z_filt, p,
                local_search_radius=int(local_search_radius),
                local_distance=int(local_distance),
                similarity_threshold=float(similarity_threshold)
            )
            for p in filtered_peaks
        ]
    else:
        refined_peaks = list(filtered_peaks)

    # Identify which peaks actually moved during refinement
    moved_mask   = [r != f for r, f in zip(refined_peaks, filtered_peaks)]
    moved_orig   = [f for f, m in zip(filtered_peaks, moved_mask) if m]
    moved_refined = [r for r, m in zip(refined_peaks, moved_mask) if m]

    # --- Build figure ---
    fig = plot_time_series(t, z_filt, title="Detected Down-peaks with AO/MV Refinement")

    # All first-pass peaks (green)
    fig.add_scatter(
        x=t[downpeaks], y=z_filt[downpeaks],
    ...
    if outlier_peaks:
        fig.add_scatter(
            x=t[outlier_peaks], y=z_filt[outlier_peaks],
            mode="markers", marker=dict(color="red", size=10, symbol="x"),
            name="Outliers (removed)"
        )

    # Refined anchors that didn't move (kept as-is, dark green)
    static_refined = [r for r, m in zip(refined_peaks, moved_mask) if not m]
    if static_refined:
        fig.add_scatter(
            x=t[static_refined], y=z_filt[static_refined],
            mode="markers", marker=dict(color="darkgreen", size=12, symbol="diamond"),
            name="Refined anchor (unchanged)"
        )

    # Original positions that were moved (orange, hollow)
    if moved_orig:
        fig.add_scatter(
            x=t[moved_orig], y=z_filt[moved_orig],
            mode="markers", marker=dict(color="orange", size=10, symbol="circle-open", line=dict(width=2)),
            name="Pass 1 peak (corrected → AO)"
        )

    # Refined positions after correction (purple)
    if moved_refined:
        fig.add_scatter(
            x=t[moved_refined], y=z_filt[moved_refined],
            mode="markers", marker=dict(color="purple", size=12, symbol="diamond"),
            name="Refined anchor (corrected to AO)"
        )

        # Draw arrows (lines) from original to refined for each moved peak
        for orig, ref in zip(moved_orig, moved_refined):
            fig.add_shape(
                type="line",
                x0=t[orig], y0=z_filt[orig],
                x1=t[ref],  y1=z_filt[ref],
                line=dict(color="purple", width=1, dash="dot")
            )

    fig.add_hline(y=lower_cut, line=dict(color='red', dash='dot'),
                  annotation_text="Lower cutoff", annotation_position="bottom right")
    fig.add_hline(y=upper_cut, line=dict(color='red', dash='dot'),
                  annotation_text="Upper cutoff", annotation_position="top right")
    ...
    return (
        fig,
        downpeaks.tolist(),
        filtered_peaks if filtered_peaks else downpeaks.tolist(),
        refined_peaks
    )
...
def update_segmentation(
    z_filt_data, refined_peaks_data, averaging_window, user_segment_width,
    relayout_data, distance, prominence, tolerance,
    ao_mv_refinement, local_search_radius, local_distance, similarity_threshold
):
    if z_filt_data is None or not refined_peaks_data:
    ...
    # Average segment overlay
    fig.add_trace(go.Scatter(
        x=avg_time, y=avg_z, mode='lines',
        line=dict(color='red', width=4), name='Average Segment'
    ))
    fig.add_scatter(
        x=avg_time[avg_dps_idx], y=avg_z[avg_dps_idx],
        mode="markers", marker=dict(color="green", size=8), name="Down-peaks"
    )
    fig.add_scatter(
        x=avg_time[avg_ups_idx], y=avg_z[avg_ups_idx],
        mode="markers", marker=dict(color="blue", size=8), name="Up-peaks"
    )
...
def update_aligned_plot(seg_idx, ref_idx, aligned_data, references_data):
    ...
    fig.add_trace(go.Scatter(
        x=time, y=amplitude, mode='lines',
        name=f'Segment {seg_idx}', line=dict(color='blue', width=2)
    ))
    fig.add_trace(go.Scatter(
        x=seg_feat_time, y=seg_feat_amp, mode='markers+text',
        name='Segment Features', marker=dict(color='green', size=10),
        text=seg_feat_labels.astype(int), textposition='top center',
        textfont=dict(size=10, color='green')
    ))
    fig.add_trace(go.Scatter(
        x=ref_time, y=ref_amplitude, mode='lines',
        name=f'Reference {ref_idx}', line=dict(color='red', width=1, dash='dash'), opacity=0.5
    ))
    fig.add_trace(go.Scatter(
        x=ref_feat_time, y=ref_feat_amp, mode='markers+text',
        name='Reference Features', marker=dict(color='red', size=6),
        text=ref_feat_labels.astype(int), textposition='bottom center',
        textfont=dict(size=9, color='red'), opacity=0.5
    ))
...
def run_prediction(n_clicks, aligned_data, model_path):
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Pre('\n'.join([
                f"  {class_names[i]}: {np.sum(all_preds == i)} segments "
                f"({np.sum(all_preds == i)/len(all_preds)*100:.1f}%)"
